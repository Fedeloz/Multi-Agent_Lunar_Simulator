...
def buffer_penalty(node, alpha=buffer_steppness):
    '''
...
class RoverGATAgent:
    ...
    def observe_state_GAT(self, node):
        ...
        local_graph = Data(x=x, edge_index=edge_index)
        local_graph.node_list = node_list
...
class Rover:
    def __init__(self, rover_id, initial_position, surface, s_color='blue', L=None):
        ...
        self.last_graph = None      # last graph observed
    ...
    def learning_step(self):
        """
        Handles learning-based packet handling.
        - If comm_policy == 2: uses DDQN agent.
        - If comm_policy == 3: uses GAT agent.
        """
        ...
        if explore_train or not import_models:
            if self.last_state is not None and self.last_action is not None:
                ...
                if self.last_action_destination.id == self.id:
                    ...
                    reward = buffer_penalty(self) + penalty_hold
                ...
                # Last time forwarded packets
                    ...
                    next_state = self.last_action_destination.current_state
                    reward = buffer_penalty(self.last_action_destination) + penalty_forward   # penalty for buffer use and for forwarding
    ...
    def update_load(self):
        ...
        """
        # 0) Learning policy
        if self.surface.comm_policy == 2 or self.surface.comm_policy == 3:
            self.learning_step()
...
class LunarSurface:
    def __init__(self, grd_size, paint_radius, comm_policy):
        ...
        if comm_policy == 2:
            ...
            if import_models:
                ...
                print('-----------------------------------')
                print(f'Graph Attention Network imported from {models_local_path}!')
            else:
                self.GATAgent = RoverGATAgent(self)
                print('-----------------------------------')
    ...
    def save_models(self, outputPath):
        ...
        """
        print('Saving models at:', outputPath)
        os.makedirs(outputPath, exist_ok=True)

        if self.comm_policy == 2:
            # Save DDQN models
        ...
        elif self.comm_policy == 3:
            # Save GAT models (PyTorch)
            agent = self.GATAgent
            torch.save(agent.policy_net, f"{outputPath}/gat_policy_net.pt")
            torch.save(agent.target_net, f"{outputPath}/gat_target_net.pt")
    ...
    def prepare_display(self, ax, combine_buffers=True, dropped_counter=False):
        ...
        for rover in self.rovers:
            ...
            if show_ttl and comm_policy == 2 or comm_policy == 3:
                ttl_value = rover.ttl
            ...
            if HTL and (comm_policy == 2 or comm_policy == 3):
                htl_value = rover.htl
        ...
        if comm_policy == 2:
            ...
            time_text = f"Time: {self.sim_time}s\nEpsilon: {self.GATAgent.epsilon:.2f}"
        else:
            time_text = f"Time: {self.sim_time}s"
